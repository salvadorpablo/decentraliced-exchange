...
module.exports = async function (deployer, _network, accounts) {
  ...
  // encontrar la forma de seedear a los distintos traders con 1000 wei de cada token
  ...
  const seedTokenBalance = async (token, trader) => {
    ...
    // transfer deposited tokens from the trader to the smart contract of the decentralized exchange
    const ticker = await token.symbol(); // SYMBOL Y NO NAME
  ...
  //create fake trades to make it seem more realistic
  ...
  //create fake orders to make it seem more realistic
  await Promise.all([
    dex.createLimitOrder(BAT, 1400, 10, SIDE.BUY, { from: trader1 }),
    dex.createLimitOrder(BAT, 1200, 11, SIDE.BUY, { from: trader1 }),
    ...
    // dex.createLimitOrder(BAT, 1200, 12, SIDE.BUY, { from: trader2 }),
  ]);
};